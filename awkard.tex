...
\\
\textbf{Corollary}\\
For any cycle $C_{j>0}$ of an ASM, $S_{a,n}$, $C_j$ has at least $n + (j-1)a$ non-activator nodes.\\
\\
\textit{Proof}\\
Let $C_j \in [S_{a,n}]$.\\
Then $|C| \geq |C_0| + (j-1-0)a = (a + n) + (j-1)a$.\\
\\
We can remove if we remove the $a$ activator nodes within the cycle, we are then left with at least $n + (j-1)a$ non-activator nodes.\\
\textit{Q.E.D}\\
\\
\\
\textbf{Lemma}\\
...
\\
Then there exists some step $p$ on which the last cycle, $C_{max}$ of the ASM is discovered.\\
\\
We know $C_{max}$ must have at least $(s-1)a$ non-activator nodes.\\
\\
...
Let $z = \prod_{i<s}|C_i|$.\\
\\
If we move the $ASM$ $z$ steps, then the position of every $C_{i<z}$ will be maintained since $z$ is divisible by the length of every $C_{i<z}$ (TODO: Prove lemma).\\
\\
...
\\
Note: the position of $C_{max}$ after moving $zt$ steps is the same as after moving $nt$ steps for any $t$. (TODO: Prove lemma).\\
\\
...
\\
Thus, $(a-jn)+n > (s-1)a \geq 2a$. (TODO: Prove there must be at least 3 cycles... have proved there must be at least 2)\\
\\
