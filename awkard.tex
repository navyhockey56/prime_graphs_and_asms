...
\tikzset{vertex/.style = {shape=rectangle,draw,minimum size=1.5em}}
\node[vertex] (a2) at  (12,0) {$p_0$};
\node[vertex] (b2) at  (14,2) {$p_1$};
\node[vertex] (c2) at  (16,0) {$p_2$};
\tikzset{vertex/.style = {shape=circle,draw,minimum size=1.5em}}
\node[vertex] (d2) at  (14,-2) {$p_3$};
...
\end{tikzpicture}
\caption{$C_{1,4}, C_{2,4}, C_{3,4}$}
The square nodes represent activators, and the circle nodes are non-activators.
\end{figure}
\textbf{Definition}
