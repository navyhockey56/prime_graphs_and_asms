...
\\
\textbf{Corollary}\\
For any cycle graph $CG_n$, a walk of length $n$ from point $p_0$ will end at point $p_0$.\\
\\
\textit{Proof}\\
Pluggin $i = 0$ into the above lemma results in $k = n - i = n - 0 = n$. Thus a walk of length $n$ from $p_0$ will end at point $p_0$ by the above.\\
\textit{Q.E.D}\\
\\
\\
\textbf{Corollary}\\
For any cycle graph $CG_n$, a wall of length $kn$, $k > 0$, from $p_0$ will end at point $p_0$.\\
\\
\textit{Proof}\\
If we move $n$ steps from $p_0$ we will return to $p_0$.\\
Assume after moving $in$, $i \geq 1$ steps from $p_0$, you have returned to point $p_0$.\\
Then, moving $n$ more steps will return to point $p_0$.\\
Thus moving $in + n = (i+1)n$ steps from $p_0$ returns you to $p_0$.\\
\textit{Q.E.D}\\
\\  
\\
\textbf{Lemma}\\
For any cycle graph $C_n$, for any $k > 0$, for any $p_i \in C_n$, a walk of length $k$ starting at $p_i$ will end at point $p_h$ where $j = i + k$ $mod$ $n$.\\
\\
\textit{Proof}\\
...
\textbf{Definition} \\
An activation cycle machine (ACM) is a directed cycle graph, $C$, containing $0 < a < |G|$ consequetive nodes called activation nodes, followed by $n = |G| - a$ non-activation nodes.
...
\\
\textbf{Lemma}\\
For $n < |C|$, $C^a<n> = [C]$ if $n$ is relatively prime to $|C|$.\\ 
\\
\textit{Proof}\\
TODO
