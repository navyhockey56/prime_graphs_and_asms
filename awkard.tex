...
\\ 
Thus, the length of $C_i$ is given by: $|C_i| = |B^{k-1}| = (k - 1) + a + n + 1 = k + a + n$.\\
\textit{Q.E.D}\\
\\
\textbf{Lemma}\\
...
\\
Then $|C_i| = k + a + n$.\\
Furthermore, $\overline{C^k_j} \geq a$, otherwise $C^k_j$ would be active and we could not have discovered $C_i$.\\
\\
Thus, $|C_j| = |C_i|$ $mod$ $|C_j| = \overline{C^k_j} \geq a$. 
\\
\textbf{Lemma}\\
