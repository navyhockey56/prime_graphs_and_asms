...
\\
\textbf{Theorem}\\
For any ACM, $C_{n,a}$, for any state, $k$, the position of state $(k + i)$ is given by talking a walk of length $i$ starting at position $\overline{C^k}$ around $CG_n$.\\
\\
\textit{Proof}\\
TODO
\\
\\
\textbf{Lemma}\\
The position of $CG_{n,a}$ on state $k$ is given by $k$ $mod$ $n$.\\
\\
\textit{Proof}\\
TODO
\\
\\
\textbf{Corollary}\\
$CG_{n,a}$ is active on state $k$ if and only if $k$ $mod$ $n < a$.\\
\\
\textit{Proof}\\
TODO
\\
